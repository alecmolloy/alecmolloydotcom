...
function Floater(config) {
  ...
  this.relationships = []
  for (
    var relationshipBetween = 0;
    relationshipBetween < config.relationshipsBetween.length;
    relationshipBetween++
  ) {
    var line1 = this.lines[
      config.relationshipsBetween[relationshipBetween].slice(0, 1).charCodeAt() - 48
    ] // Store first line
    var line2 = this.lines[
      config.relationshipsBetween[relationshipBetween].slice(1, 2).charCodeAt() - 48
    ] // Store second line
...
Floater.prototype.updateAnchorPosition = function(anchor) {
  for (var iDimension = 0; iDimension < this.dimensions; iDimension++) {
    ...
    if (
      anchor.vector[dimension] >= this.field[dimension] / 2 ||
      anchor.vector[dimension] <= 0 - this.field[dimension] / 2
    ) {
...
Floater.prototype.createAnchor = function() {
  ...
  for (var iDimension = 0; iDimension < this.dimensions; iDimension++) {
    var dimension = this.dimensionNames[iDimension]
    vector[dimension] = Math.round(
      Math.random() * this.field[dimension] - this.field[dimension] / 2,
    )
...
Floater.prototype.checkLines = function(destroyedAnchor) {
  for (var i = 0; i < this.lines.length; i++) {
    if (
      this.lines[i].anchor1.index === destroyedAnchor.index ||
      this.lines[i].anchor2.index === destroyedAnchor.index
    ) {
...
Floater.prototype.checkRelationships = function(destroyedLine) {
  for (var relationship = 0; i < this.relationships.length; i++) {
    if (
      this.relationships[relationship].line1.index === destroyedLine.index ||
      this.relationships[relationship].line2.index === destroyedLine.index
    ) {
...
Floater.prototype.createSegments = function(relationship, segments) {
  ...
  for (var line in this.relationships[relationship]) {
